...
<a href="admin.php"> VIEW CONTACT FORM</a>
  <a href="updatecontact.php" class="active">UPDATE CONTACT FORM</a>
...
<div class="main">
  <h2>ADMIN SPACE: </h2>
  <h2>UPDATE ANY RECORD FROM THE CONTACT FORM</h2>
  <table id="view">
  <tr>
    <th>ID</th>
    <th>NAME</th>
...
if($conn -> connect_error){
  ...
  if($result -> num_rows > 0){
      while($row = $result -> fetch_assoc()){
          ...
          <td>" . $row["COUNTRY"] . "</td>
          <td>" . $row["SUBJECT"] . "</td>
          <td><a href='upcon.php?rn= $row[ID] &em= $row[NAME]  &e=$row[EMAIL] &c=$row[COUNTRY] &s=$row[SUBJECT]'> Update</a></td>
          </tr>";
      }
      echo "</table>";
  }
